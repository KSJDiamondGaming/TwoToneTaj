  ...
  return (
    <main className="community-page">
      <section className="community-hero">
        ...
        <aside className="community-discord-card"><div className="community-card-glow" /><img src={site.assetUrls.primaryLogo || logo} alt={`${site.brand.name} logo`} /><span>{dataState === 'live' ? 'Discord Connected' : 'Discord Ready'}</span><strong>{serverName}</strong><p>Join the official community for streams, clips, gaming and updates.</p><a href={site.socials.discord} target="_blank" rel="noopener noreferrer">Open Discord</a></aside>
      </section>

      <section id="community-live" className="community-stats-grid">{stats.map(([label, value]) => <article className="community-stat-card" key={label}><span>{label}</span><strong>{value}</strong></article>)}</section>

      <section className="community-dashboard-panel"><div><span className="eyebrow">Community Updates</span><EditableField as="h2" fieldId="communityPage.dashboardTitle" label="Community updates title" value={page.dashboardTitle} policy={site.editorPolicy} /><EditableField as="p" fieldId="communityPage.dashboardText" label="Community updates text" value={page.dashboardText} policy={site.editorPolicy} kind="textarea" /></div></section>

      <section className="community-section-head"><span className="eyebrow">Inside The Discord</span><EditableField as="h2" fieldId="communityPage.areasTitle" label="Community areas title" value={page.areasTitle} policy={site.editorPolicy} /></section>
      <section className="community-area-grid">{areas.map(([icon, title, text]) => <article className="community-area-card" key={title}><span>{icon}</span><h3>{title}</h3><p>{text}</p></article>)}</section>

      <section className="community-values-panel"><div className="community-values-copy"><span className="eyebrow">Community Standards</span><EditableField as="h2" fieldId="communityPage.valuesTitle" label="Community values title" value={page.valuesTitle} policy={site.editorPolicy} /><EditableField as="p" fieldId="communityPage.valuesText" label="Community values text" value={page.valuesText} policy={site.editorPolicy} kind="textarea" /></div></section>
    </main>
  )
}
