    ...
    <main className="home">
      ...
      <section className="home-grid">
        <article className="panel about-panel">
          <EditableField as="h2" fieldId="home.aboutTitle" label="About section title" value={site.home.aboutTitle} policy={site.editorPolicy}>
        ...
        <article className="panel schedule-panel">
          <EditableField as="h2" fieldId="home.scheduleTitle" label="Schedule section title" value={site.home.scheduleTitle} policy={site.editorPolicy}>
            <img src={playIcon} alt="" />
            {site.home.scheduleTitle}
          </EditableField>
          <div className="schedule-list">
            {streamSchedule.map(([day, time]) => (
              <p key={day}><strong>{day}</strong><span>{time}</span></p>
            ))}
          </div>
          <div className="schedule-timezone">
            <small>Times shown in</small><strong>GMT</strong>
            <p>Schedule is subject to change. Follow on social media for updates!</p>
          </div>
        </article>

        <article className="panel twitch-panel">
          <EditableField as="h2" fieldId="home.twitchTitle" label="Twitch section title" value={site.home.twitchTitle} policy={site.editorPolicy}>
            <img src={twitchIcon} alt="" />
            {site.home.twitchTitle}
          </EditableField>
          ...
          <div className="twitch-meta-card">
            ...
            <span>Follow on Twitch for live alerts, community streams, and the latest gaming sessions.</span>
            <a href={site.socials.twitch} target="_blank" rel="noreferrer">Follow on Twitch</a>
          </div>
        </article>

        <article className="panel youtube-panel">
          <div className="youtube-panel-head">
            <EditableField as="h2" fieldId="home.youtubeTitle" label="YouTube section title" value={site.home.youtubeTitle} policy={site.editorPolicy}>
              <img src={youtubeIcon} alt="" />
              {site.home.youtubeTitle}
            </EditableField>
            <a href={site.socials.youtube} target="_blank" rel="noreferrer">View all videos →</a>
          </div>
          <YouTubeFeed />
        </article>
      </section>
      ...
      <section className="socials-panel panel">
        ...
        <div className="socials-grid">
          {socialLinks.map(([icon, title, handle, url]) => (
            <a className="social-card" href={url} target="_blank" rel="noreferrer" key={title} aria-label={`Open ${site.brand.name} ${title}`}>
              <img src={icon} alt="" /><strong className="social-handle">{handle}</strong>
            </a>
          ))}
        </div>
      </section>
      ...
      <section className="expect-panel panel">
        ...
        <div className="expect-grid">
          {expectCards.map(([icon, title, text]) => (
            <article className="expect-card" key={title}><img src={icon} alt="" /><strong>{title}</strong><p>{text}</p></article>
          ))}
        </div>
      </section>
      ...
      <section className="merch-panel">
        <div className="merch-card">
          <div className="merch-content">
            ...
            <EditableField as="h2" fieldId="home.merchTitle" label="Merch section title" value={site.home.merchTitle} policy={site.editorPolicy} />
            <EditableField as="p" fieldId="home.merchText" label="Merch section text" value={site.home.merchText} policy={site.editorPolicy} kind="textarea" />
            <div className="merch-actions"><Link to="/merch" className="btn primary">View Merch</Link></div>
          </div>
        </div>
      </section>
    </main>
